...
//=============================================================================
// 改修内容まとめ
//  1. カメラ : update() のスプライン評価ロジックを evaluate_at_time() に分離。
//              再生中以外でも任意の時刻を評価できるようにし、
//              タイムラインのドラッグによる「スクラブ（巻き戻し/早送り再生）」を実現。
//  2. UI    : CustomUI (CustomWidgets) を全面的に採用してテーマを統一。
//              保存/読込/削除/各種ウェイポイント操作の結果はトースト通知で
//              フィードバックするようにし、何が起きたかが一目で分かるようにした。
//  3. タイムライン : CustomUI::EditableTimeline を Playback タブに統合。
//              各ウェイポイントの再生位置をひし形キーフレームとして可視化し、
//              再生ヘッドのドラッグでカメラ位置をスクラブプレビューできる。
//  4. 保存   : JSON 完全移行。拡張子を ".json" に統一し、ウェイポイントに加えて
//              ループ再生設定・ノード間の再生時間も JSON へ保存/復元するようにした。
//              （以前は再生設定が draw_tab_playback() 内の static 変数で保持されており、
//                同クラスの全インスタンスで設定が共有されてしまうバグがあったため修正）
//=============================================================================
